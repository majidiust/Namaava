		...
		doInterval:function(){
			...
			var timer = setInterval(function(){
				...
				success:function(result,textStatus,jqXHR ){
					if(result.Status){
						//I am Here
						$("#WebinarConnectivityMessage").html('وبینار در حالت آنلاین است.');
						$("#OfflineLed").hide();
						$("#OnlineLed").show();
					}
					else{
						...
						{
							$("#WebinarConnectivityMessage").html('وبینار در حالت آفلاین است.');
						$("#OfflineLed").show();
						$("#OnlineLed").hide();
						}
					}
	...
	webinarInfoView: Backbone.View.extend({
		...
		initialize:function(){
			...
			if(presentorInfo != null){
				...
				tab1.find('h4').html('عنوان وبینار :'+ '<br />'+info.name);
